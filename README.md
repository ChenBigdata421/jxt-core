...
- v1.7.11 - eventbus 告警结构性哑火根修（logger 注入 + sarama 包级日志接线）+ outbox ack 标记超时放宽 + reliable §16.12 索引合并（三项独立交付；无 Go API 破坏性变更）。【eventbus】第八案核查（2026-09-19）：kafkaEventBus 构造默认 logger=zap.NewNop()，partition pipeline 的 stall WARN（p.log=k.logger）非 nil 恰好绕过 warnStall 的 nil 静默检查——告警因此结构性哑火。① 新增 kafkaEventBus.SetLogger（须在消费者/订阅启动前调用）+ factory 层 SetBusLogger（全局总线注入，鸭子类型断言 *kafkaEventBus；非 Kafka 实现安全 no-op；nil 忽略；幂等）。② 轮10冻结案（2026-09-18）：dispatcher trigger 断供 39 分钟零日志——会话终结/分区分发失败/连接错误等关键路径仅经 sarama 包级 Logger（默认 io.Discard）输出，全写 /dev/null；新增 wireSaramaLogger（sync.Once，NewKafkaEventBus 与 reinitializeConnection 两处接线）桥接到 stderr，微秒时间戳与 gorm 错误行同源可交叉比对；EVENTBUS_SARAMA_LOG=0 显式关闭。【outbox】ackMarkFlushTimeout 5s→10s——MarkBatchAsPublished/MarkAsPublished 的单次 flush/单事件超时放宽一倍，降低 DB 抖动（锁等待/慢盘）下的误超时回退。【reliable §16.12 索引合并（2026-09-15）】MySQL 无 partial index，PROCESSING 行进入所有 status 前导索引，而 idx_due/idx_unresolved/idx_retention 的消费者从不查询 PROCESSING 行——纯死重写放大（PG 侧用 partial index 规避）。Tier 1：idx_ops + idx_retention → idx_ops(status, first_seen_at, updated_at)——DeleteSettledBefore 的 updated_at 为第 3 列，无法 range 但索引覆盖（SUCCEEDED/DISCARDED range 上 index-only scan）。Tier 2：idx_due + idx_unresolved → idx_replay(status, next_attempt_at, handler_id, first_seen_at)——FindEligibleHeads 前两列完美 range + 无 filesort（与原 idx_due 等价）；RetryAgeSeconds/PendingCounts 丢失松散索引扫描（handler_id 降至第 3 列）退化为覆盖扫描 + 临时表聚合，30s 探针绝对开销毫秒级。合并后 INSERT 写 4 个索引项（3 status 前导 + idx_aggregate），UPDATE status→SUCCEEDED 触及 6 次树操作（3 棵树 × 删+插），约 -40% 写放大；状态迁移的二级索引维护相对附录 Z 前累计 7→3（约 -57%）。配套：model.go 的 idx_due 出 GORM tag（PG 侧 partial 同名不同形，双源原则索引只由双方言 migration DDL 承载）、idx_ops 列序 priority 3→2；recovery.go 保留清理注释同步；repotest 两 EXPLAIN 门禁断言从 idx_due/idx_unresolved 迁移到 idx_replay（镜像 ref/range access 断言合并为单索引格；FrozenAggregates 外层校准格放宽为 idx_replay/idx_ops 任一）。⚠ 仅随 CREATE TABLE 生效（本迁移无 ALTER 机制，OV⑧b 同缺口）；既存 MySQL 库由 evidence 侧版本化迁移补建（DROP 旧 idx_due/idx_unresolved/idx_retention + 重建 idx_replay/idx_ops 三列形）；PG 侧 idx_due 为 partial 同名不同形、idx_retention_* partial 索引不受影响，新库自动新形态。【破坏性】无 Go 符号/API 变更；schema 仅索引形态（新建库自动新形态；既存库见上方迁移说明）。【消费方】全部 Kafka 服务升级即获告警可见性（stall WARN/poison alerter 不再哑火）+ sarama 终结路径 stderr 日志；evidence-management/file-storage-service 获索引写放大下降（既存 MySQL 库需 evidence 侧迁移）。verify：go build ./sdk/pkg/eventbus/... ./sdk/pkg/outbox/... ./sdk/pkg/reliable/... 全绿。
- v1.7.10 - 附录 Z 索引瘦身 + MarkFailed 伪装盲区根修（R5）+ 瞬态错误毫秒级重试（D1/P0）+ 依赖批量升级（gin/gorm/zap 等）。【附录 Z 索引瘦身（jxt-benchmark/docs/analysis 死锁风暴根因分析，Z.4/Z.6，2026-09-01 内核侧落地）】① `idx_handler (handler_id, status)` 整体移除（两方言 migration DDL + GORM tag 三处同步）——零专属消费者（探针全部 status 打头走 idx_unresolved；opsvc List/Stats 走 idx_ops；handler 定位由 uk_event_consumption 覆盖），且是 2026-08-23 死锁风暴 dump 里 T2 手持 29 把锁的检索路径；状态迁移的二级索引维护 7 → 6（约 -14% 写放大）。② `idx_ops` 去前导 tenant_id（3 列 → 2 列）、`idx_aggregate` 同去（8 列 → 7 列）——一库一租户下 tenant_id 基数为 1（opsprobe 明确「正确性依赖一库一租户」），前导常量列纯属写放大浪费；新列序与 FindEligibleHeads NOT EXISTS 谓词逐列对齐。PG FrozenAggregates 外层由 BitmapAnd(idx_unresolved ∩ idx_handler) 退化为 idx_unresolved 单腿（原同规模 seqscan=off 对照 +27% 代价、等值墙钟，opsprobe 校准记录留痕）；EXPLAIN 门禁同步校准为「status 打头的索引承载外层（idx_unresolved/idx_due/idx_ops 任一）」，镜像 MySQL 侧口径。⚠ 既存 MySQL 库的旧索引由 evidence 侧版本化迁移 DROP（内核 CREATE TABLE IF NOT EXISTS 对已建表 no-op，OV⑧b 同流程）；PG 新库自动建为瘦索引。【R5 伪装盲区根修】mark.go 的 MarkFailed 前置 SELECT（读 m.Attempt）原稿对 err 一律 `return ErrConflict`——1213/1205/连接抖动/ctx 超时这类 DB 错误被伪装成 fencing 哨兵，下游 Classify(ErrConflict)=Unrecoverable → RecordTerminal 撞 PROCESSING → 分区冻结（2026-08-23 事故链）。修复：仅 `ErrRecordNotFound`（First 配 WHERE 谓词 → 行不存在/已不归我）映射 ErrConflict，其余原样上抛——与同函数下方 UPDATE、RecordTerminal 的既有模式对齐；conformance 新增 `MarkFailed_SelectCtxError_Propagates`（预取消 ctx 驱动 SELECT 分支 + 行必须仍 PROCESSING）+ `MarkFailed_StaleToken_StillErrConflict`（钉住 fencing 语义不收窄）双子测。【retryTransient 瞬态重试（D1/P0，R5 的第二层）】新增 `gormshared/retry.go`：对 classifier.ClassifyDriver 判 ClassRetryable 的瞬态错误（MySQL 1213/1205/1040/1053/1452、PG 40001 等，不硬编码错误码）做 3 次有界重试、50~200ms 随机退避——InnoDB 回滚牺牲者后冲突窗口已消失，毫秒级重试成功率极高（2026-08-23 事故里 31 秒烧完 5 次 attempt 预算的消息本可就地吸收）；不消耗 attempt 预算（基础设施层自愈与 maxAttempts 语义解耦）；ctx 取消/超时立即上抛；非瞬态码（POISON/CONFLICT/未识别）单次即返——绝不把 fencing 冲突重试掉。MarkSucceeded/MarkFailed 的三个 CAS 语句全部接入（WHERE 带全 token/id+attempt，0 行 → ErrConflict 不属于瞬态不会被重试，重试安全）。配套 5 个单测（真实 `*mysqldriver.MySQLError` 驱动 errors.As 真路径：收敛/耗尽上抛原始错误/ctx 取消即返/冲突码不重试/未识别码不重试）。【依赖批量升级】gin v1.7.7→v1.10.0（连带 validator v10.25.0、locales/universal-translator、gin-contrib/sse v1.0.0）、gorm v1.24.2→v1.25.5、gorm.io/driver/mysql v1.4.4→v1.5.2、go-sql-driver/mysql v1.7.1→v1.9.0（连带 edwards25519）、zap v1.17.0→v1.24.0、prometheus/client_golang v1.18.0→v1.21.1、ginkgo v2.27.1、gomega v1.38.2、jwt/v4 v4.5.2、websocket v1.5.0、protobuf v1.36.7。gorm.io/driver/postgres 保持 v1.4.5（pgx/v4）不动——升 ≥v1.5.x 会切 pgx/v5，v1.7.9 的双代断言已兼容但按最小变更原则留待消费方驱动。【破坏性】无 Go API 变更；schema 仅索引形态（新建库自动新形态；既存库见上方迁移说明）。【消费方】evidence-management / file-storage-service 升级后获得死锁风暴下的事务自愈（瞬态 1213/40001 不再烧 attempt 预算）+ 索引写放大下降；既存 MySQL 库需 evidence 侧 DROP 旧 idx_handler/idx_ops/idx_aggregate 并按新列序重建。verify：`go build ./...` 全绿；`go test ./pkg/reliable/store/gormshared/ ./pkg/reliable/adapters/eventbus/ ./pkg/reliable/store/postgres/` 全绿；repotest 双方言一致性套件（含 EXPLAIN 门禁新口径）217s 全绿。
- v1.7.9 - PGClassifier 双代 PgError 断言修复 + FromRawMeta 直测（单点修复；无 API/schema 变更）。【根因】`store/postgres/classify.go` 的 `IsDuplicateKey`/`ClassifyDriver`/`ErrorCode` 此前只断言 pgx/v4 的 `*pgconn.PgError`（typed `errors.As`）——而 gorm.io/driver/postgres ≥ v1.5.x 底层换成了 pgx/v5，产出的错误是 `github.com/jackc/pgx/v5/pgconn` 的同名异包结构体，typed 断言跨包不命中：SQLSTATE 23505 不再进 ClassConflict、唯一键重复检测（claim.go D3）恒 false。受影响使用方：file-storage-service（driver pin v1.5.4 → pgx/v5）；evidence-management（driver v1.4.5 → pgx/v4）不受影响。【后果实证】数据安全（fail-closed + broker 重投自愈：重复投递撞唯一键被当普通错误上抛 → 分区不 ACK → 重投后走已有行路径），但代价是多一次投递往返 + 异常类别误标（error_code 列拿不到 SQLSTATE）。根因经两个服务真实依赖树上的 probe 实证（非纸面推断）。【修复】抽取 `pgErrorCode` helper 串联双断言——先 v4 `*pgconn.PgError` 再 v5 `*pgx/v5/pgconn.PgError`，三处调用点（ClassifyDriver/IsDuplicateKey/ErrorCode）统一走它。选双断言而非类型别名：两代结构体字段兼容（Code/Message/Detail...），各断各的即可，且两包都已在模块图内（v4 本就是 direct require；v5 因 driver≥1.5 的使用方从 indirect 提升为 direct——go.mod 相应调整）。用 build tag 分文件同理多余：两包同时可导入。【测试】RED→GREEN：`TestPGIsDuplicateKey_PgxV5`（含 `fmt.Errorf("%w")` 包裹链路，模拟 driver 错误被 gorm/服务层包一层的真实形态）+ `TestPGClassifierAndErrorCode_PgxV5`（ClassifyDriver 23505→ClassConflict、ErrorCode 40P01 提取）；不覆盖 gorm.ErrDuplicatedKey 哨兵臂——kernel pin gorm v1.24.2 无此哨兵（v1.25.0+ 才有，且需 driver 开 TranslateError；fs 生产 gorm.Open 未开，错误保持 v5 原生形态）。【FromRawMeta 直测补口】PR-7 Phase C review 发现：B10 去重删除了 file-storage 的本地直测后，`adapters/eventbus.FromRawMeta`（LIVE 路径的 RawMeta→DeliveryMeta 投影，file-storage OnDelivery 直接调用）在 kernel 零直测——C1/C6 有序重复 header 往返只剩 FromPoisonMessage 一侧的间接覆盖。新增 `TestFromRawMeta_PreservesFieldsAndHash` 钉住全字段投影：与 FromPoisonMessage 的关键差异是 PayloadHash 为【透传】而非计算（RawMeta 由 pipeline 预计算）——断言原样穿过、不被重算；headers 保有序+允许重复 key 不合并不重排；HeaderPair 类型契约（key.go:70，不依赖 eventbus 包）。【破坏性】无。【消费方】file-storage-service 升级后 v5 唯一键冲突即恢复 D3 短路（省一次重投往返 + error_code 正确标注）；evidence-management 无行为变化。verify：`TestPGIsDuplicateKey_PgxV5` / `TestPGClassifierAndErrorCode_PgxV5` RED→GREEN；`go test ./pkg/reliable/store/postgres/... ./pkg/reliable/adapters/...` 全绿；`go build ./...` 全绿。分支按用户 2026-08-30 决策不单独打标：折入下一个有真实 driver 出现的内核 tag（本条目先行记录）。
- v1.7.8 - PG back-CAS 42P18 修复（production bug；单点修复，无 API/schema 变更）。【根因】v1.7.7 终评修复轮 ②（79f6aa6，cause-append back-CAS）给 `opsvc.casBackToQuarantined` 的 error_message 追加表达式 `LEFT(CONCAT_WS(' | ', error_message, CONCAT('replay[', replay_attempts + 1, ']: ', ?)), 2000)` 留下裸参数占位符——CONCAT/CONCAT_WS 在 PostgreSQL 是 variadic `any` 函数，planner 无法给裸参数定型 → `SQLSTATE 42P18: could not determine data type of parameter $1`，整条 back-CAS UPDATE 在 PG 上必败；MySQL 隐式定型不受影响（故双方言系统测试的 MySQL 腿全绿、PG 腿红）。该轮的 PG leg 未复跑是漏网原因。【影响（PG 全量）】QuarantineReplay 的每条失败路径（decode 错误 / live-claim 冲突 409 / AlreadySettled 非 SUCCEEDED 分诊 / attempt-cap 拒绝）的 back-CAS 全部失败 → 隔离行滞留 REPLAYING、`replay_attempts` 永不增长、`QuarantineReplayMaxAttempts=5` 上限在 PG 上完全失效（仅剩 10min watchdog sweep 把行扫回 QUARANTINED 兜底，但计数器仍不增长，上限依旧绕过）。复现：`cd sdk && go test -tags system -count=1 -run TestQuarantineReplay_System_ClaimBackCycle ./pkg/reliable/opsvc/` → postgres 子测试断言 `REPLAYING ≠ QUARANTINED` 失败且日志含 42P18。【修复】沿用方言分支先例（repotest/setup.go `dropSQLFor`）按 `db.Dialector.Name()` 取两份表达式串、共享参数与语义：PG `CAST(? AS text)`（text 为 PG 本位类型）、MySQL `CAST(? AS CHAR)`（MySQL 不认 text 目标类型）——GORM 无法在单个 gorm.Expr 内分流方言，两串是唯一可移植形态；两种形态均经真实 Go 绑定参数路径在 mysql:8.0 + postgres:16 逐字验证（追加串 `"orig cause | replay[1]: <sanitize(cause)>"` 与 `replay_attempts +1` 计数行为双方言一致）。函数头补 v1.7.8 修复注。【同类残留清扫】全仓 grep 非测试 Go 代码中参数化 `CONCAT`/`CONCAT_WS`/`COALESCE`/`NULLIF`/`GREATEST`/`LEAST`——仅本处一处命中；gormshared/recovery.go 的 `COALESCE(error_class,'RETRYABLE')` 只接受字面量参数（PG 安全），无同类第二处。【破坏性】无。【消费方】evidence-management / file-storage-service 以 v1.7.8 落地 PR-7 Phase B/C 的，PG 环境的 QuarantineReplay back-CAS 与 5 次上限恢复生效；无 schema/迁移动作。verify：claim-back 周期用例双方言红→绿；opsvc `-tags system` 双方言全绿；`go test ./pkg/reliable/... ./pkg/outbox/...` 15 包全绿；`reliable_deps_gate.sh` ALL GATES GREEN。
- v1.7.7 - PR-7 运维闭环内核增量（ops closure Phase A；additive，向后兼容；无 Go API 破坏性变更）。【opsprobe】`store/gormshared` 新增三个运维只读探针（standalone 函数接 `*gorm.DB`，非接口方法——store.Store/QuarantineStore 接口冻结不变）：`RetryAgeSeconds`（§10 ①，per-handler 最老 RETRY_SCHEDULED 年龄，Go 侧 `now.Sub(MIN(first_seen_at))` 可移植计算，无方言 EXTRACT）；`FrozenAggregates`/`FrozenAggregatesSQL`（§10 ④，与 `EarlierUnsolvedSiblingSQL` 同源三段式 earlier-than 谓词的镜像方向 const，锁步耦合注释纪律 R4-H，含 aggregate-less 守卫防 ''='' 恒真误报 OV②）；`PendingCounts`（status×handler 分组计数，仅未终结两态）。配套 `idx_unresolved` 索引（PG partial `(handler_id,first_seen_at) WHERE status IN 两态` / MySQL `(status,handler_id,first_seen_at)` 复合，D7=6A——无则三探针每 30s 全表扫）+ repotest 6 格 EXPLAIN 门禁（100k fixture，双方言断言索引扫描非 Seq/ALL；PG FrozenAggregates EXISTS 构建侧 Seq Scan 为已记录 known-gap：三态 IN 谓词是二态 partial 索引谓词的结构性超集，seqscan=off 全索引计划 +26.8% 成本，属成本模型选择而非索引缺陷；探针自身扫描已严格门禁）。⚠ MySQL 内核迁移无 ALTER 机制（OV⑧b/D1），既存 MySQL 环境需 evidence 侧版本化迁移补 `idx_unresolved` 索引 + `raw_message_quarantine.replay_attempts`/`updated_at` 两列（内核 CREATE TABLE 注释已指向）。【§3.2 自动回收 + §10 保留】`RecoverExpiredProcessing`（lease 过期超 stuckAfter 且带 payload 的 PROCESSING 行 CAS 转回 RETRY_SCHEDULED，WHERE 保留 lease 复验防砸在途消费者活租约 OV③；payload-NULL 行只计数交 broker 重投 §2.1；不直接写异常行——经返回行明细由服务侧 store.RecordAnomaly 落 STUCK_PROCESSING，保 metricsStore seam OV④a）+ `DeleteSettledBefore`（SUCCEEDED/DISCARDED 按期清理，DEAD_LETTER 永不自动清）。【ObserveExpiredLeases 语义修正 OV④b】返回值由「扫描行数」改为「新插入行数」（批量 INSERT 的 RowsAffected；MySQL 经 gorm OnConflict DoNothing→`ON DUPLICATE KEY UPDATE id=id` 冲突行 RowsAffected=0，PG ON CONFLICT DO NOTHING 同理）——扫描语义下单条持久孤儿每 30s tick 重复计数（120/h）误触 >10/h 告警；接口签名不变（不破坏消费方显式 fake）。⚠ 若消费方 DSN 设 `clientFoundRows=true`，MySQL 冲突行报 1，静默退回扫描语义。【opsvc QuarantineReplay C①】`WithRegistry`（仅校验 HandlerID 已注册，绝不直调 Handle）+ `WithEnvelopeDecoder`（服务侧注入 eventbus 解码；opsvc 保持 eventbus-free J2）两选项（`ServiceOption` 变参追加，两参调用不变），缺任一 fail-closed 返回 `ErrQuarantineReplayUnsupported`。重放=「毕业」而非第三执行路径（OV⑤）：quarantine 行 CAS QUARANTINED→REPLAYING 后以行内 RawValue 重建 delivery、TryClaim 仲裁——`Claimed` 分支接 `MarkFailed(ClassRetryable, payload=RawValue)`（评审裁决为必要：TryClaim-only 残留 PROCESSING/payload-NULL 行对 EligibleHeadsSQL（仅扫 RETRY_SCHEDULED）不可见、RecoverExpiredProcessing 不管 payload-NULL、且隔离时 offset 已 ACK 无 broker 重投，MarkFailed 是唯一持有 token 且写全 chk_retry_due 三件套的路径→行落 RETRY_SCHEDULED 由既有重放调度器排干，聚合门/attempt 上限/头排序全量生效；ReplayUnsafe handler 经此正确落 DEAD_LETTER 双闸）；`AlreadyProcessing`→行回 QUARANTINED + 409 live-claim-active；`replay_attempts` 列计数，`QuarantineReplayMaxAttempts=5` 封顶前查（CAS 前，行不动）；REPLAYING 超 10min watchdog 可重占。resolve 不覆盖 error_message（保 DLQ 原始隔离成因，对齐 gormshared.MarkResolved 纪律）。真实 DB 系统测试钉住三个 CAS 语句（claim→back / claim→resolve / watchdog，双方言）。【outbox C②】`OutboxRepository` 新增 `FindDeadLettered`（status='dead_lettered' 按 dead_lettered_at DESC,id DESC 分页，tenantID<=0 全租户运维视图，**含已 dlq-notified 行**——与 FindUnnotifiedDeadLettered 的差异即运维清单的意义）+ `CountDeadLettered`；四服务均自定义窄接口、经构造器取 core 实现，接口加宽安全（执行期 grep 复核）；仓内 MockRepository 获真实内存语义供后续运维 REST 测试。【B11 门禁对等】gates_test.go 补 nats-io 根禁令 + 三个 scoped 子包依赖门禁的 Go 化（此前仅 bash 脚本有；变异双向验证：违规侧红、允许侧绿）。【PR-2 leftover A】①-⑥ 注释纠错与测试补口（adapter Errorf-nil-sink 注释、seedRowWithStatus chk 注释、EarlierUnsolvedSibling 三臂 conformance 种子补齐（arm-2 分区/偏移 + arm-3 first_seen 回退 + 空边界 + 混合因果，附 pgx 裸 NULL 占位符左移陷阱注释）、Discard Fatalf 变量、ResolverError 10 方法全覆盖、跨租户隔离断言扩展）。【终评修复轮（9-pass review，79f6aa6）】打标签前对 PR-7 全窗口跑 9 轮评审（6 专家：testing/maintainability/security/performance/data-migration/api-contract + Claude 对抗 + Codex 对抗 + Codex 结构化门禁 + Red Team；Codex 结构化 P1 经占位符逐字符清点裁决为误报——19 `?` 对 19 绑定参数，'AUTO' 落 replay_mode）。修复三个 critical：① QuarantineReplay AlreadySettled 假成功——TryClaim 的 default 臂对【全部】非 PROCESSING 状态返回 AlreadySettled（含 DEAD_LETTER/RETRY_SCHEDULED，key.go 文档只承诺「已终结」），毕业 #1 的 MarkFailed 落地但 qrResolve 输掉并发 CAS → 行回 QUARANTINED → 消费侧随后合法死信 → 操作者再点 replay 即「绿色成功 + 隔离行 RESOLVED」＝事故记录伪造；修复：AlreadySettled 分支经 `byKeyStatus`（uk_event_consumption 三键回读 status，seam 化与 qrClaim 同因——单测 harness 是 sentinel *gorm.DB）分诊，仅 SUCCEEDED 走 RESOLVED，其余回 QUARANTINED + `ConflictError{settled but not SUCCEEDED}`（新 conflictReasonSettled，五来源）；新增 TestQuarantineReplay_AlreadySettledDeadLetterRefused 钉住。② casBackToQuarantined 覆盖原始隔离成因——与 I-3 纪律（RESOLVED 不写 error_message 保事故成因）自相矛盾：失败回迁（解码失败/租户不符/live-claim 409/TryClaim/MarkFailed 错误）无条件覆盖 error_message，烧完 5 次的毒消息只剩「live consumer holds the lease」；修复改追加语义 `LEFT(CONCAT_WS(' | ', error_message, CONCAT('replay[', replay_attempts + 1, ']: ', ?)), 2000)`（双方言 CONCAT_WS/LEFT 兼容；截尾旧轮次、计数器才是完整台账），fake 同步镜像；新增 TestQuarantineReplay_BackAppendsNotOverwritesCause 钉住头部保留。③ FindDeadLettered limit 陷阱——gorm v1.24.2 clause/limit.go 仅在 *limit>=0 时生成 LIMIT 子句：limit<0 整句丢弃 LIMIT（全表死信含 payload 无界加载）、limit=0 恒空页，且 MockRepository（limit<=0 返空）与 gormshared 钳制约定（<=0→100）三方分叉；修复：impl 钳制 limit<=0→100 / offset<0→0，MockRepository 对齐，接口注释钉死约定，LimitOffsetPaging 测试补 4 个边界断言。④ 保留清理索引（performance）：`DeleteSettledBefore` 的 DELETE 谓词（status+updated_at 两臂 OR）原先无任何含 updated_at 的索引支撑＝每次清理必然全表扫——MySQL 新增 `idx_retention(status,updated_at)`、PG 新增两条 partial `idx_retention_succeeded/_discarded`（updated_at WHERE status=...，仅随 CREATE TABLE/IF NOT EXISTS 生效，存量 MySQL 同走 OV⑧b evidence 侧补建）；DeleteSettledBefore 头注释补幂等台账语义（被删事件重投即首见重执行，标准 at-least-once）。【终评 minors】QuarantineDetail/QuarantineRow 补 `ReplayAttempts`/`UpdatedAt`（操作者点击前可见「4/5 次」与 REPLAYING 停留时长，ToRow 同步映射）；QuarantineReplay 解码失败改返回静态分类错误（decoder 包 eventbus.FromBytes→jsoniter，其 Iterator.ReportError 会把原始输入 ±50 字节窗嵌进错误文本——毒载荷 PII 无审计外泄向量；详细 cause 仅经 SanitizeForStorage 进 error_message），TestQuarantineReplay_DecodeError 改钉「原文不上抛 + cause 落库」双断言；recoverRowSQL 的 error_class/error_code 改 `COALESCE(error_class,'RETRYABLE')`/`COALESCE(error_code,'LEASE_EXPIRED_RECOVERED')`（经 ClaimForReplay 重放过的行保留上一轮 MarkFailed 真实失败元数据，不再被 LEASE_EXPIRED_RECOVERED 占位符覆盖到烧完 attempt 上限——§10 fingerprint 聚合指向真实缺陷；chk_retry_due 仅要求非空，COALESCE 满足）；opsprobe 三函数补「无 tenant 谓词、正确性依赖一库一租户」作用域注释；ConflictError 注释三来源→五来源；opsvc dto.go 包头纠偏（quarantine replay 已在本包 + import 清单补 replay/gormshared）；row.go settleBack→casBackToQuarantined 指针纠错；service.go 隐形软连字符（U+00AD，破坏 grep/复制）清除；mysql/migration.go「8.0.29+ 才有的语法」事实纠错（ADD COLUMN IF NOT EXISTS 是 MariaDB 语法，MySQL 任何版本无；8.0.29 加的是 INSTANT 加列）；M-3 影响面纠正——未迁移存量 MySQL 库上 `QuarantineModel.Record`（隔离写入路径，adapters write-before-ACK）INSERT 新列即报 1054 → 分区阻塞，**早于任何 QuarantineReplay 点击**（原注释把首击点错标在 replay）；evidence 侧前向引用（2026082300002/3、ops_service.go）标注「PR-7 服务侧待落地，内核合入时尚未创建」；opsprobe 两文件 '' 注释触发 gofmt 排印引号重写（真内容级违规，非 CRLF 假象）已 gofmt -w；runner_test OV④b 透传断言补锚注释。【已知遗留（记录未修）】PG FrozenAggregates EXISTS 构建侧 3 态 partial 索引可进一步消除（README 前文 known-gap 的补救路径）；FrozenAggregates 逐死信行返回（limit 100 界内单热聚合可能饱和）；RecoverExpiredProcessing 候选 SELECT 无 LIMIT + stuckAfter<=0 无守卫（负值把 cutoff 推到未来会砸活租约——调用方控参）；watchdog 谓词依赖所有 REPLAYING 写入方用 UTC；ObserveExpiredLeases 500 上限 + inserted 语义下 >500 孤儿积压零信号（需配 backlog gauge）；attempt 上限把 AlreadyProcessing 409 也计入（活跃期可能误锁重放）；tests/** hermetic 文件在 CI 不跑的「-race 混跑」组合下有 logger.Setup 全局竞态（测试基建债，非内核缺陷）。【破坏性】无 Go 符号/API 破坏；outbox 接口为纯加宽（消费方无实现方）。消费方：evidence-management + file-storage-service 待 PR-7 Phase B/C 以 v1.7.7 依赖落地运维 REST；security/process/tenant 不导入 reliable/outbox 新符号无需升级。⚠ 既存 MySQL 环境升级需按上述 OV⑧b/D1 迁移清单执行（PG 自愈）。verify：go build ./... + broker-free 单测 + `-tags system`（DSN 驱动，注意：多包并行跑 system 套件需 `-p 1`——repotest.Setup 无跨包串行化，共享 DSN 库的并行 DROP/CREATE 会互相踩）+ deps gate 全绿；终评轮后复验：hermetic 全量 `go test -p 1 ./...` 70 包 0 FAIL（含 repotest 双方言 564s、eventbus 105s、perf 137s 套件）；`-race` 按 CI test-race.yml 精确清单逐项全绿（broker-free 12 包 + eventbus 子集 + reliable 内核 7 包 + config 解码守卫 + repotest `-race` 309s）；module-hygiene 单模块门禁绿；新增 3 个回归测试（DeadLetterRefused / BackAppendsNotOverwritesCause / LimitOffset 边界）全部通过。
- v1.7.6 - 测试基建与并发修复：默认 `go test ./...` 关卡重构为 hermetic（无 broker/零 hang），外加 eventbus 并发缺陷根修与全量 flake 清零（additive；无 Go API 破坏性变更）。【hermetic 门禁】全部依赖 broker 的 `_test.go`（`sdk/pkg/eventbus` 2 个 + `tests/eventbus/{function,reliability,performance}` 21 个 + `tests/outbox/function` 3 个，共 26 文件）加 `//go:build integration`——默认 `go test ./...` 在无 Kafka/NATS/RedPanda 的机器上完整跑完不 hang（此前 `sdk/pkg/eventbus` 整包必 hang、perf 套件按设计跑 10+ 分钟被误判 hang）；broker 覆盖走 `go test -tags=integration ./...`，CI `test-regression.yml` 已改用该 tag（否则打标会静默丢覆盖）。配套：perf 套件 `OrderChecker`/`NewOrderChecker` 抽到未打标的 `ordercheck_helper_test.go`（hermetic 用例依赖）；删除 2 个已废弃 `// +build ignore` 死文件。【Metrics 数据竞争根修】并发 `Publish` 下 `Metrics` 计数非原子自增丢增量（`TestEventBusManager_ConcurrentPublish` 的 ≥100 断言 flake、`TestMemoryEventBus_ConcurrentPublish` 在 -race 下必报）：`eventBusManager` 以专用 `metricsMu`（与 `mu` 解耦，updateMetrics 在 Publish 持 RLock 时被调无法升级）守护全部 metrics 读写；`memoryEventBus` 计数器改 `atomic.AddInt64`；**关键**：manager 不再与内层 bus 共享同一 `*Metrics`（`NewMemoryEventBus` + `initMemory` 两处 `m.metrics = bus.metrics` 别名，使 mutex-plain++ 与 atomic 两种同步方式在同一对象上互踩）——两测试在 `-race ×8` 下零竞争，`test-race.yml` 解除对二者的 KNOWN EXCLUSION 并重新纳入。【SubscribeEnvelope 指标对齐】envelope 订阅两条路径（快速路径透传 + 回退路径仅包反序列化）均绕过 `updateMetrics`，manager 侧 envelope 消费计数恒 0——此前仅靠已拆除的共享别名掩盖；现统一包上与 `Subscribe` 一致的指标/采集器包装。【flake 根修】① 健康检查用例启动顺序竞态：先启 publisher（启动即首发）后启 subscriber，memory 总线无持久化丢弃首发，默认 `Interval=2min` 下必败——改为先订阅后发布 + 条件等待（`TestHealthCheckConfigurationApplication/DefaultConfiguration`、`TestHealthCheckMessageFlow`）；② 绝对墙钟性能断言全部改负载无关相对比较：domain/event `MarshalDomainEvent` 与 outbox `NewOutboxEvent` 均以 `encoding/json` 同事件为基线（安全系数 2×/5×），不再在 CI/重载机器上误判；③ `TestPartitionConfigComparison` 陈旧断言对齐库的有意行为（`CanAutoFix` 恒 false——CreatePartitions 已移除、分区收敛归 infra bootstrap）；④ repotest `MarkResolved_CAS`/`MarkResolved_CtxError_Propagates` 用例传无效 `tenantID=0` 被 S3 多租户守卫正确拒绝（store 实现无误，用例早于守卫）——改用有效租户 1，MySQL+PG 双方言恢复全绿（CI test-race run#21 红的根因）；⑤ `pre_subscription` 用例不再直戳未导出 `allPossibleTopics`（绕锁、不设 snapshot），改走公共 `SetPreSubscriptionTopics`。【vet 清零】`go vet ./...` 0 告警（jwtauth 死 fmt.Errorf、ws.go Upgrade 失败路径 context 泄漏补 cancel、memory_test range 拷贝锁）。【gofmt】401 个告警确认为 core.autocrlf 的 CRLF 假象（提交 blob 为 LF 且 gofmt 干净），未改。【破坏性】无 Go 符号/API 破坏；测试行为变更仅为打标/修 flake，断言语义不变。消费方：全部服务受益于默认关卡可跑性 + Metrics/SubscribeEnvelope 计数正确性；broker 测试需 CI 以 `-tags=integration` 执行（已配置）。verify：`go test -p 1 ./...` 47 包全绿；`-race` ConcurrentPublish 套件 ×8 零竞争；`-tags=integration` function/reliability/outbox-function + 12.6min perf 套件（22 用例）全绿；repotest 双方言 ok；go build/vet clean。
- v1.7.5 - v1.7.4 reliable 内核的双轮评审加固（additive，向后兼容；无 Go API 破坏性变更）。对 v1.7.4 的 §10/§7 上层包（opsvc + DLQ adapter + sanitize + store 读）跑了两轮独立评审（gstack-review + ce-code-review 6 人格），落地评审发现的缺陷与补全；两轮均独立确认 v1.7.1 的 W1（claim.go 内联续占不升 attempt）与 W2（隔离区 hash 截断不对称）非 jxt-core 缺陷。【sanitize 跨拷贝对齐】v1.7.4 显式延后的「不改 gormshared.sanitizeMsg」本版补齐：把根 SanitizeForStorage 的引号密码脱敏（password="x"/'x'）port 到内核 sanitizeMsg（fingerprint.go）——原 split-brain 使主表路径（RecordTerminal→event_consumption.error_message）照旧泄露引号密码（隔离区路径已修），本版两份 sanitize 统一。【opsvc 信息泄露收敛】List 由直接透出 []store.Row（泄露 ReplayAuthID 一次性重放 bearer / ClaimID fencing token / 审批人 PII）改 []Detail 白名单投影；Detail 摘 ClaimID；BatchReplay Err/Conflict 三态互斥 + 补 ConflictReason（保留 409 来源 sibling/D12/row_version）；requireTenant/读路径守卫 ==0→<=0（与 DLQ adapter 同源）。【store 租户纵深】QuarantineStore List/GetByID/MarkResolved 补 tenantID<=0 守卫（最敏感表原先仅 WHERE-scope）。【schema 跨方言对等】mysql raw_key VARBINARY(512)→LONGBLOB（两表），与 PG BYTEA + 同表 payload/raw_value 一致——>512B Kafka key 原在 MySQL 严格模式 INSERT 失败卡分区、PG 不受影响；⚠ 既存 MySQL 环境需手动 ALTER TABLE ... MODIFY raw_key LONGBLOB（CREATE TABLE IF NOT EXISTS 对已建表 no-op）。【测试】conformance 加 confLargeRawKeyPersists（字节级回读）+ confLargeQuarantineRawKeyPersists（隔离区表对等）；opsvc 加 List 白名单 leak 断言 + BatchReplay 三态/ConflictReason；adapter 加 FromPoisonMessage 直测。【破坏性】无 Go 符号/API 破坏；schema 仅 MySQL raw_key 列类型放宽（既存环境见 ALTER）。消费方 evidence-management + file-storage-service（EventBusDLQAdapter 消费方）升级即获 sanitize 主表修复 + raw_key 对等；security/process/tenant 不导入 reliable 无需升级。verify：go build ./... + go vet + broker-free 单测全绿；conformance 大 key 用例 DB-gated 编译通过。
