...
- **最佳实践方案（第二轮复评采纳 1B，desired-state / runtime-state 分离）**：`subscriptions` map 是**期望状态**（该 bus 应消费哪些 topic），是重连的唯一恢复源，**重连全程不清空**；`activeTopicHandlers` 是**运行时状态**，随新消费组清空重建。重连走新增的内部 `subscribeInternal`——绕过公开 `Subscribe` 的 `LoadOrStore` 去重守卫（该守卫只为保护公开 API 不被调用方重复订阅，不该拦内部重连），重新激活 handler + 在新消费组上重启消费循环。⚠️ **`restoreSubscriptions` 不得 read-then-wipe**：它从未清空的 `subscriptions` 遍历恢复即可（这也是 reconcile-loop 的标准姿势：读期望状态对齐运行时，从不清空期望状态）。**否决先前推荐的"清空再恢复"**：清空的唯一动机是绕守卫，却把恢复源一起清掉，制造期望状态瞬时为空窗口 + 恢复正确性与清空/恢复时序耦合。
...
```
  ...
  ③ 清空 activeTopicHandlers（运行时状态）; subscriptions（期望状态）不清空 ◀── (C1, 采纳 1B)
  ④ restoreSubscriptions: 遍历未清空的 subscriptions, 走 subscribeInternal(绕过 LoadOrStore 守卫)重激活 handler + 在新组重启消费
...
- **`k.mu` 仅短暂持有状态变更**：①stop / ③清 `activeTopicHandlers`（**不清 `subscriptions`**，采纳 1B）/ ④restore / ⑤置 `consumerStarted` 各自"取 `k.mu` → 改 → 释"，**不跨** ② 网络重建、**不跨** ⑤ warmup。
...
- **最佳实践方案**：goroutine 所有权 + 优雅关闭。加 `sync.WaitGroup`（或 stop chan）；`Close`/`reinitializeConnection` 先 signal stop、关 producer、`wg.Wait()`；`Publish` 经 generation/RWMutex 门禁，不能向正在关闭的 producer 发；`Close()` drain `publishResultChan`（或通知 Outbox 消费方关闭）；`reinitializeConnection` spawn 新 pair 前先停旧 pair 并 `wg.Wait()`；**修 tenant close(ch) 竞态（第二轮复评采纳 2A）**：竞态在**发送侧**——`sendResultToChannel` 在 RLock 外做 `tenantChan <- result`（`kafka.go:3481`），而 `close` 已在写锁下（`kafka.go:3562`）。3477 释放读锁、3481 发送前若 `UnregisterTenant` 抢到写锁 close 该 chan → send-on-closed → panic。**修法**：将租户非阻塞 select 发送整段移进 `tenantChannelsMu.RLock` 保护区（发送本为 `select`+`default` 非阻塞，持 RLock 不会阻塞 `UnregisterTenant`；full 时 default 命中后先释 RLock 再走全局 fallback）。⚠️ **否决两个无效/劣化选项**："close 在持锁下与 `delete` 原子化"**已是现状**（3552/3562/3563）、对发送侧竞态无作用，不采纳；`recover` 兜 send-on-closed 会**静默丢这条 ACK**（Outbox 行不被 mark → 重投），是掩盖非消除，不采纳。（A-1，并入 Phase 1，与 Critical 重连重构同改。）
...
- **重连（C1-C6 + M3，Phase 1）**：编排级单测断言状态机顺序 / **C1 正向计数不变量：重连后 `subscriptions` 恢复集合 == 重连前集合（数量+内容）且每个 topic 在新组重激活 handler（非仅"无 already-subscribed 错误"，采纳 3A）** / `activeTopicHandlers` 重建 / 配置 parity + kill-switch（env=off 不触发 reconnect）；e2e 断言断连重连后新消息可消费、goroutine 不泄漏、`Close()` 不阻塞；`go test -race`。
...
| 回归 | 测试 | 类型 | 依赖 broker? |
|---|---|---|---|
| C1 restoreSubscriptions 必败 | mockbroker：重连前订阅 N(>0) 个 topic，重连后断言 `subscriptions` 集合 == 重连前（数量+内容）且每个 topic 在新组重激活 handler（**正向计数不变量**，非"无 already-subscribed 错误"——后者对"静默零恢复"会误判通过，采纳 3A） | 单测 | 否（mockbroker） |
...
| M3-tenant UnregisterTenant close 竞态 | 单测：in-flight ACK 期 unregister tenant 不 panic（修法：租户非阻塞 select 发送移进 `tenantChannelsMu.RLock` 保护区，采纳 2A；否决 close-持锁-原子化[已是现状无效]与 recover[静默丢 ACK]） | 单测 | 否 |
...
|--------|---------|-----|------|--------|----------|
| Eng Review | `/plan-eng-review` | 架构/代码/测试/性能 | 2 | CLEAR | 第1轮：Arch 4（1A/2A/3A/4A）+ Tests 3（5A/6A/7A）全采纳。第2轮（code-grounded 独立复评，已核验 C1/C2/C3/C4/H2/M3-tenant 于 kafka.go 属实）：3 条新 finding 全采纳——F1[Arch]/F2[Code]/F3[Test] |
...
**第2轮 code-grounded 复评 findings（均已折入 spec）：**
- **F1 [Arch, 8/10] → 1B**：C1/C5 原"清空 subscriptions 再 restore"与 `restoreSubscriptions` 从 live map 读恢复源冲突——清空即静默零恢复。改为 desired/runtime 分离：`subscriptions`（期望状态）永不清空、仅清 `activeTopicHandlers`，重连走 `subscribeInternal` 绕公开去重守卫（reconcile-loop 姿势）。
- **F2 [Code, 9/10] → 2A**：M3 ④ tenant close 竞态在**发送侧**（`sendResultToChannel` @3481 锁外发送，`close` @3562 已在锁内）。改为把非阻塞 select 发送移进 `tenantChannelsMu.RLock`；否决"close 持锁原子化"（已是现状、无效）与 recover（静默丢 ACK）。
- **F3 [Test, 8/10] → 3A**：C1 回归断言"无 already-subscribed 错误"对 F1 的"静默零恢复"会误判通过。改为正向计数不变量：重连后 subscriptions 集合==重连前（数量+内容）且每 topic 在新组重激活 handler。

- **VERDICT**：ENG CLEARED —— 两轮工程评审 + outside voice findings 全部采纳并入 spec。第2轮独立复评 code-grounded 核验了核心诊断（C1 never-Delete、C3 bare NewConfig、H2 `len==0` 漏判、M3-tenant 发送侧竞态）属实，并抓出 arch→code→test 一条自洽的连锁盲点（F1/F2/F3）——均已修正。
- **交叉模型一致性**：outside voice 独立确认 spec 若干诊断准确（legacy 严格 offset 有序、H1 压缩、C1 never-Delete）；第2轮复评额外发现 3 项前几轮漏掉的自洽缺陷，无悬而未决分歧。
