...
- 日期：2026-07-30
- 范围：jxt-core（仅）
- 状态：待评审（已并入评审反馈 v4：v3 的 D2/D3/D4 + v4 2nd eng pass 的 D2=A `reliable.live_enabled` 耦合校正）
- 目标 tag：v1.1.69
...
- **激活面（非默认路径；发布决策已定 = D2/A）**：「默认行为零变化」仅对 *未写* `pipeline` 段的配置成立。evidence-management `command/config/settings.yml` 与 `query/config/settings.yml` **当前已提交** `eventbus.kafka.consumer.pipeline.enabled: true`（windowSize 32 / 64）——死开关是眼下唯一把它们留在 legacy 串行路径上的闸门。本修复一旦被它们吃到（§6：本地 `replace ../../jxt-core` 让开发态即时生效），partition pipeline + DLQ seam 即在 windowSize 32/64 上 **激活**，等同非目标 #2 推迟的「灰度发布」；§4.3 校验拦不住（两者 `sessionTimeout=10s` → 10/2=5s > 默认 `FlushTimeout=4s`，校验通过 → 静默激活而非 fail-fast）。
  - **决策（D2=A，打 tag 前执行，耦合多键翻转——非一行）**：与 evidence-management 协调，在 **v1.1.69 同发布窗口**把 `pipeline.enabled` 置 `false`。但 `pipeline.enabled: false` **单独不是 no-op**：修好开关后总线与 evidence-management 共读同一 key（re-coupling，见 §6），而 evidence-management command 的可靠订阅走 PR-3 Delivery 路径，当 `pipeline.enabled != true` 时 R2-6 fail-fast **拒绝订阅**（`command/internal/infrastructure/eventbus/subscriber.go:162`），`reliable.live_enabled` 又默认 `true` 且无已提交 key（`command/internal/infrastructure/reliable/config.go:119`，R2-3）。故 D2=A **必须同窗口同翻 `reliable.live_enabled: false`**——把 command 的可靠消费从 PR-3 Delivery 切到 PR-0 `SubscribeEnvelopeWithDLQ` fallback（`reliable/config.go:29-32`），后者 pipeline 关着也能订阅、走 legacy 串行、DLQ seam 休眠 = 与今天一致，这才是真 no-op。**保留 `reliable.scheduler_enabled: true`**（RETRY_SCHEDULED 行已在 broker 端 ACK、不会重投，只有 scheduler 能排空——`reliable/config.go:35-37`；翻为 false 即 R2-3 数据丢失模式）。query：同样翻 `pipeline.enabled=false`，并（若 query 走 Delivery 可靠路径）同翻 `live_enabled=false`；command 已核实，query 接线未在本次 review grep 范围内，须核实。这是 evidence-management **自己设计的回滚模式**（两 flag 拆分的目的即此——`reliable/config.go:23-27`），非新机制。真正的激活作为受 4 前置条件（幂等 handler、DLQ 接线、扩分区、eager assignor）约束的**独立后续发布**——其中幂等 handler 审计即 `TODOS.md` F1，仍 open。**发布说明须载明此耦合翻转动作**，否则 evidence-management 下次 bump pin 即静默激活。
...
- **evidence-management R2-6 fail-fast 自动消解（但 re-coupling 有代价，见 §5 D2=A）**：evidence-management 的 `LoadReliableConfig` 直接读 `settings` 的 `eventbus.kafka.consumer.pipeline.enabled` 做 fail-fast 判定（R2-6）。修完 jxt-core 后，运行期总线也吃同一个 key——「fail-fast 读的值」与「运行期实际值」第一次一致，Task 6 review 报的那个 Critical 会随这一处修复自动消解。**但 re-coupling 的代价**：`pipeline.enabled` 现在同时是 (a) 总线 `consumeWithPipeline` 的闸门、(b) evidence-management R2-6 订阅 fail-fast 的闸门——同一个 key。死开关时代「总线休眠 + reliable 订阅」的不可能中间态消失，变成 both-on 或 both-off；故 D2=A（key=false）会同时让总线休眠**且**让 reliable 在 Delivery 路径上拒绝订阅——必须同翻 `live_enabled=false` 切到 PR-0 fallback（见 §5 D2=A）。进阶（非必需）：可让 evidence-management 改读已解析结构体 `config.AppConfig.EventBus.Kafka.Consumer.Pipeline.Enabled`，更稳。
- **版本与 replace**：tag = **v1.1.69**（jxt-core v1.1.x 顺序，PR-2 = v1.1.68）。evidence-management 用本地 `replace ../../jxt-core`，jxt-core 一提交、evidence-management 开发态立即吃到；release 时再把 pin 改 v1.1.69。**发布协调（D2=A + D5=A）**：evidence-management 在 pin 到 v1.1.69 的**同一发布**里，把 `command`/`query` settings.yml 的 `pipeline.enabled` 置 `false`，并**剥离/改注 3 个 timing 键**（`flushTimeout`/`dlqTimeout`/`stallWarnInterval`——无解码目标、静默丢弃，见 §5 D5），确保 v1.1.69 以 no-op 上线；激活另作受 4 前置条件约束的独立发布。
- **协调范围（P2）—— 3 个 Kafka 消费服务**：除 evidence-management `command`/`query` 外，`process-management`（`type: kafka`，`config/settings.yml`）是第 3 个 jxt-core Kafka 消费方。它**当前无 `pipeline:` 段** → v1.1.69 下不激活；但它**照搬了 command 的 `groupId`**（`evidence-command-consumer-group`，`:102`），未来若再 copy 一份 `pipeline:` 段是现实激活隐患。发布协调须追踪 3 个服务，而非 2 个。（范围外披露：process-management 与 evidence-management command 共用 `evidence-command-consumer-group` 是 consumer-group 冲突——process-management 侧的 bug，非本 PR，须另行路由修复。）
