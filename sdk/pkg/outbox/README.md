...
ASYNC（独立 goroutine）：
  ACK listener → broker ACK 回来 → repo.MarkAsPublished（逐条标 published）
```

**特点**：
- 依赖 broker（Kafka/RedPanda 或 NATS JetStream）
- 标记清理由 ACK listener 异步处理
- **必须** 启动 `StartACKListener`（由业务服务 wiring 层负责）
- `SyncSemanticsPublisher` 标记未实现 → `PublishBatch` 不会走同步标记路径

### 性能特性

无论同步还是异步，以下两条路径在 `PublishBatch` 内共享且都是批量操作：

| 操作 | 批量实现 | 效果（BatchSize=100） |
|---|---|---|
| `filterPublishedEvents`（幂等检查） | `FindPublishedByIdempotencyKeys` — `WHERE idempotency_key IN (…)` | N 次 SELECT → **1 次 SELECT** |
| `MarkBatchAsPublished`（同步标记） | 单条 `UPDATE WHERE id IN (…) AND status='pending'` | N 次 UPDATE → **1 次 UPDATE**（sync only） |

异步 publisher 跳过 `MarkBatchAsPublished`，额外节省一次 DB 批量写入。
