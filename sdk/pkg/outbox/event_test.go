...
func TestNewOutboxEvent(t *testing.T) {
	...
	// 验证 Payload（现在存储的是完整的 DomainEvent）
	var decodedEvent jxtevent.BaseDomainEvent
	if err := jxtjson.Unmarshal(event.Payload, &decodedEvent); err != nil {
		t.Fatalf("Failed to unmarshal payload: %v", err)
	}

	// 验证 DomainEvent 的字段
	if decodedEvent.EventType != "UserCreated" {
		t.Errorf("Expected event type to be 'UserCreated', got '%s'", decodedEvent.EventType)
	}
	...
	if payloadMap["name"] != "test" {
		t.Errorf("Expected payload name to be 'test', got '%v'", payloadMap["name"])
	}
}
...
func TestOutboxEvent_Clone(t *testing.T) {
	...
	event := &OutboxEvent{
		...
		Payload:       jxtjson.RawMessage(`{"name":"test"}`),
		Status:        EventStatusPending,
		RetryCount:    0,
		MaxRetries:    3,
		CreatedAt:     now,
...
func TestNewOutboxEvent_IDGeneration(t *testing.T) {
	event, err := NewOutboxEvent(
		...
		"User",
		"UserCreated",
		createTestDomainEvent("UserCreated", "aggregate-1", "User", map[string]interface{}{"name": "test"}),
	)
...
func TestIdempotencyKey(t *testing.T) {
	event, err := NewOutboxEvent(
		...
		"Archive",
		"ArchiveCreated",
		createTestDomainEvent("ArchiveCreated", "aggregate-123", "Archive", map[string]interface{}{"name": "test"}),
	)
